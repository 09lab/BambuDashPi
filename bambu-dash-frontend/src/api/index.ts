...
const apiPost = async <T, B = any>(uri: string, body: B, config?: AxiosRequestConfig): Promise<ApiResponse<T>> => {
    try {
        const response = await client.post<ApiResponse<T>>(uri, body, config);
        return response.data;
    } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
            const axiosError = error as AxiosError<ApiResponse<any>>; 
